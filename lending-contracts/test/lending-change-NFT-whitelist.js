...
describe("Lending | Change NFT Whitelist", function () {
  ...
  it("Only owner can change an NFT address whitelisted", async function () {
    await expect(
      this.lending
        .connect(alice)
        .changeNFTWhitelist(true, this.erc721_I.address, aggregator)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should change an NFT address whitelisted", async function () {
    await expect(
      this.lending.changeNFTWhitelist(true, this.erc721_I.address, aggregator)
    )
      .to.emit(this.lending, "NFTWhitelistedChanged")
      .withArgs(true, this.erc721_I.address, aggregator);
  });
});

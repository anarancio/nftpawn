...
describe("Lending | Set BasketDeployer address", function () {
  ...
  it("Only Lending contract owner can set basketdeployer", async function () {
    await expect(
      this.lending.connect(lender1).setBasketDeployerAddress(lender1.address)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  it("Sholud revert if basketdeployer address is address(0)", async function () {
    await expect(
      this.lending.setBasketDeployerAddress(ethers.constants.AddressZero)
    ).to.be.revertedWithCustomError(this.lending, "BasketDeployerAddressZero");
  });
  it("Should revert if basketdeployer address is the same as lending contract address", async function () {
    await expect(
      this.lending.setBasketDeployerAddress(this.lending.address)
    ).to.be.revertedWithCustomError(
      this.lending,
      "BasketDeployerAddressNotLendingContract"
    );
  });
  it("Should set basketdeployer address", async function () {
    await this.lending.setBasketDeployerAddress(lender1.address);
    expect(await this.lending.basketDeployerAddress()).to.equal(
      lender1.address
    );
  });
});

...
describe("Lending | Set NFT Floor Price Time", function () {
  ...
  describe("Set Accepted Floor Price time", function () {
    it("Only owner can set accepted floor price", async function () {
      await expect(
        this.lending.connect(alice).setAcceptedFloorPriceTime(100)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    it("Should revert if floor price time is set to 0", async function () {
      await expect(
        this.lending.setAcceptedFloorPriceTime(0)
      ).to.be.revertedWithCustomError(
        this.lending,
        "AcceptedFloorPriceTimeLowZero"
      );
    });
    it("Should set accepted floor price time", async function () {
      await this.lending.setAcceptedFloorPriceTime(100);
      expect(await this.lending.acceptedFloorPriceTime()).to.equal(100);
    });
  });
});
